...
@Repository
public interface CuentasPorCobrarRepository
                extends JpaRepository<CuentasPorCobrar, Long>, JpaSpecificationExecutor<CuentasPorCobrar> {
        ...
        @Query(value = "SELECT * FROM `cuentas_por_cobrar` WHERE asesor_cartera_id = :idAsesor AND total_obligatoria > 0 AND mora_obligatoria > 0 ORDER BY dias_vencidos DESC", countQuery = "SELECT COUNT(*) FROM `cuentas_por_cobrar` WHERE asesor_cartera_id = :id_asesor AND total_obligatoria > 0 AND mora_obligatoria > 0 ORDER BY dias_vencidos DESC", nativeQuery = true)
        ...
        @Query(value = "SELECT DISTINCT cuentas_por_cobrar.* FROM cuentas_por_cobrar JOIN gestiones ON gestiones.cuenta_cobrar_id = cuentas_por_cobrar.id_cuenta_por_cobrar WHERE cuentas_por_cobrar.asesor_cartera_id = :idAsesor AND (SELECT MAX(gestiones.fecha_gestion) FROM gestiones WHERE gestiones.cuenta_cobrar_id = cuentas_por_cobrar.id_cuenta_por_cobrar) < :fechaInicial AND cuentas_por_cobrar.mora_obligatoria > 0 AND cuentas_por_cobrar.total_obligatoria > 0", 
                        countQuery = "SELECT COUNT(*) FROM (SELECT DISTINCT cuentas_por_cobrar.* FROM cuentas_por_cobrar JOIN gestiones ON gestiones.cuenta_cobrar_id = cuentas_por_cobrar.id_cuenta_por_cobrar WHERE cuentas_por_cobrar.asesor_cartera_id = :idAsesor AND (SELECT MAX(gestiones.fecha_gestion) FROM gestiones WHERE gestiones.cuenta_cobrar_id = cuentas_por_cobrar.id_cuenta_por_cobrar) < :fechaInicial AND cuentas_por_cobrar.mora_obligatoria > 0 AND cuentas_por_cobrar.total_obligatoria > 0) AS subquery;", nativeQuery = true)
        ...
        @Query(value = "SELECT cuentas_por_cobrar.* FROM `cuentas_por_cobrar` LEFT JOIN gestiones ON gestiones.cuenta_cobrar_id = cuentas_por_cobrar.id_cuenta_por_cobrar LEFT JOIN clasificacion_gestion ON gestiones.clasificacion_gestion_id = clasificacion_gestion.id_clasificacion_gestion LEFT JOIN acuerdo_pago ON acuerdo_pago.id_clasificacion_gestion = clasificacion_gestion.id_clasificacion_gestion WHERE clasificacion_gestion.clasificacion = 'ACUERDO DE PAGO' AND acuerdo_pago.is_active = true AND acuerdo_pago.asesor_cartera_id = :asesor AND acuerdo_pago.fecha_compromiso = :compromiso ORDER BY cuentas_por_cobrar.id_cuenta_por_cobrar DESC", countQuery = "SELECT COUNT(*) FROM `cuentas_por_cobrar` LEFT JOIN gestiones ON gestiones.cuenta_cobrar_id = cuentas_por_cobrar.id_cuenta_por_cobrar LEFT JOIN clasificacion_gestion ON gestiones.clasificacion_gestion_id = clasificacion_gestion.id_clasificacion_gestion LEFT JOIN acuerdo_pago ON acuerdo_pago.id_clasificacion_gestion = clasificacion_gestion.id_clasificacion_gestion WHERE clasificacion_gestion.clasificacion = 'ACUERDO DE PAGO' AND acuerdo_pago.is_active = true AND acuerdo_pago.asesor_cartera_id = :asesor AND acuerdo_pago.fecha_compromiso = :compromiso ORDER BY cuentas_por_cobrar.id_cuenta_por_cobrar DESC", nativeQuery = true)
        Page<CuentasPorCobrar> obtenerAcuerdosPagoActivosAndFechaCompromiso(@Param("asesor") Long asesor, @Param("compromiso") Date compromiso, Pageable pageable);


        @Query(value = "SELECT cuentas_por_cobrar.* FROM `cuentas_por_cobrar` LEFT JOIN gestiones ON gestiones.cuenta_cobrar_id = cuentas_por_cobrar.id_cuenta_por_cobrar LEFT JOIN clasificacion_gestion ON gestiones.clasificacion_gestion_id = clasificacion_gestion.id_clasificacion_gestion LEFT JOIN acuerdo_pago ON acuerdo_pago.id_clasificacion_gestion = clasificacion_gestion.id_clasificacion_gestion WHERE clasificacion_gestion.clasificacion = 'ACUERDO DE PAGO' AND acuerdo_pago.is_active = true AND acuerdo_pago.asesor_cartera_id = :asesor AND acuerdo_pago.fecha_compromiso < now() AND cuentas_por_cobrar.total_obligatoria > 0 ORDER BY cuentas_por_cobrar.id_cuenta_por_cobrar DESC", countQuery = "SELECT COUNT(*) FROM `cuentas_por_cobrar` LEFT JOIN gestiones ON gestiones.cuenta_cobrar_id = cuentas_por_cobrar.id_cuenta_por_cobrar LEFT JOIN clasificacion_gestion ON gestiones.clasificacion_gestion_id = clasificacion_gestion.id_clasificacion_gestion LEFT JOIN acuerdo_pago ON acuerdo_pago.id_clasificacion_gestion = clasificacion_gestion.id_clasificacion_gestion WHERE clasificacion_gestion.clasificacion = 'ACUERDO DE PAGO' AND acuerdo_pago.is_active = true AND acuerdo_pago.asesor_cartera_id = :asesor AND acuerdo_pago.fecha_compromiso < now() AND cuentas_por_cobrar.total_obligatoria > 0 ORDER BY cuentas_por_cobrar.id_cuenta_por_cobrar DESC", nativeQuery = true)
        Page<CuentasPorCobrar> obtenerAcuerdosPagoActivosVencidos(@Param("asesor") Long asesor,  Pageable pageable);

        @Query(value = "SELECT COUNT(*) FROM `cuentas_por_cobrar` LEFT JOIN gestiones ON gestiones.cuenta_cobrar_id = cuentas_por_cobrar.id_cuenta_por_cobrar LEFT JOIN clasificacion_gestion ON gestiones.clasificacion_gestion_id = clasificacion_gestion.id_clasificacion_gestion LEFT JOIN acuerdo_pago ON acuerdo_pago.id_clasificacion_gestion = clasificacion_gestion.id_clasificacion_gestion WHERE clasificacion_gestion.clasificacion = 'ACUERDO DE PAGO' AND acuerdo_pago.is_active = true AND acuerdo_pago.asesor_cartera_id = :asesor AND acuerdo_pago.fecha_compromiso < now() AND cuentas_por_cobrar.total_obligatoria > 0 ORDER BY cuentas_por_cobrar.id_cuenta_por_cobrar DESC", nativeQuery = true)
        Integer obtenerAcuerdosPagoActivosVencidosCount(@Param("asesor") Long asesor);
